...
Since the poisson regression models the log mean of the poisson regression, the mean number of homicide victims for black people is estimated to be exp(`r round(intercept.poisson,digits = 2)`) = `r round(exp(intercept.poisson), digits = 2)` and exp(`r round(intercept.poisson, digits = 2)` `r round(race.poisson, digits = 2)`) = `r round(exp(intercept.poisson + race.poisson), digits = 2)` for white individuals. Those averages underestimate the observed values (section \@ref(EDA)). The ratio of the mean responses is `r round(exp(sum.mod.poisson$coefficients[1,1] )/exp(sum.mod.poisson$coefficients[1,1] +  sum.mod.poisson$coefficients[2,1]), digits = 2)` (black/white) and `r round(exp(sum.mod.poisson$coefficients[1,1] +  sum.mod.poisson$coefficients[2,1])/exp(sum.mod.poisson$coefficients[1,1]), digits = 2)` (white/black). This means that, on average, a black person knows `r round(exp(sum.mod.poisson$coefficients[1,1] )/exp(sum.mod.poisson$coefficients[1,1] +  sum.mod.poisson$coefficients[2,1]), digits=2)` times more homicide victims than a white person notice that `r round(exp(sum.mod.poisson$coefficients[1,1] +  sum.mod.poisson$coefficients[2,1])/exp(sum.mod.poisson$coefficients[1,1]),digits = 2)` = 1/`r round(exp(sum.mod.poisson$coefficients[1,1] )/exp(sum.mod.poisson$coefficients[1,1] +  sum.mod.poisson$coefficients[2,1]), digits=2)`.
...
Indeed, one important assumption in a Poisson model, is that the mean is equal to the variance. The variance in the data is `r var(data$resp)` (`r unname(unlist(summ[1,5]))` for black and `r unname(unlist(summ[2,5]))` for white respondents). Figure \@ref(fig:GOF1) below shows there is overdispersion (the real variance in red is larger than the simulated variance). 

```{r include=TRUE, message = FALSE}
sim.mod.poisson <- simulateResiduals(mod.poisson, plot = T, seed = 2022)
```
```{r include=TRUE, message = FALSE}
...
The regression results show that the dispersion parameter $\phi$ is estimated to be `r round(sum.quasip$dispersion, digits = 2)` which is much larger than one (Poisson model assumes it to be one).
...
# Conclusion {#CON}
Table \@ref(tab:comparemodels) shows the different models' estimate coefficients and Akaike Information Criterion (AIC). The zero-inflated model parameters cannot directly compared to the other three models. Therefore, estimated mean and variance for both races are also included. The mean and variance for the number of homicide victim a black or white person know is closest to the sample mean and variance in the zero-inflated poisson model and the zero-inflated negative binomial is a close second. The zero-inflate Poisson model also has the lowest AIC. Figure \@ref(fig: comparemodels2) compares the different model graphically. It is clear that the poisson model performs the worst, especially for black people. The zero-inflated models are the closest to the true data where zero-inflated Poisson seems to slightly outperform the zero-inflated negative binomial model; it almost entirely overlaps with the sample probabilities. 

```{r comparemodels, include=TRUE, message = FALSE}
comparison <- data.frame(poisson = c(intercept.poisson, intercept.std.poisson, race.poisson, race.std.poisson, NA, NA, NA, NA, meanblack.poisson, meanblack.poisson, meanwhite.poisson, meanwhite.poisson, aic.poisson),
                         negbin = c(intercept.nb, intercept.std.nb, race.nb, race.std.poisson, NA, NA, NA, NA, meanblack.nb, varblack.nb, meanwhite.nb, varwhite.nb, aic.nb),
                         quasil = c(intercept.quasip , intercept.std.quasip, race.quasip, race.std.quasip, NA, NA, NA, NA, meanblack.quasip, varblack.quasip, meanwhite.quasip, varwhite.quasip, aic.quasip),
                         zip = c(intercept.zip , intercept.std.zip, race.zip, race.std.zip, intercept.zero.zip , intercept.zero.std.zip, race.zero.zip, race.zero.std.zip, meanblack.zip, varblack.zip, meanwhite.zip, varwhite.zip, aic.zip),
                         zinb = c(intercept.zinb , intercept.std.zinb, race.zinb, race.std.zinb, intercept.zero.zinb , intercept.zero.std.zinb, race.zero.zinb, race.zero.std.zinb, meanblack.zinb, varblack.zinb, meanwhite.zinb, varwhite.zinb, aic.zinb)) 
rownames(comparison) <- c('Intercept', 'Intercept std', 'racewhite', 
                          'racewhite std','Intercept ', 'Intercept std ', 
                          'racewhite ', 'racewhite std ', 'mean black', 
                          'variance black','mean white', 'variance white', 
                          'AIC')
comparison %>%
  kable(booktabs = T, 
        caption = "Comparison of all models.",
        row.names = TRUE,
        col.names = c('Poisson','Negative binomial','Quasi likelihood','Zero-inflated  Poisson','Zero-Inflated Negative binomial'),
        digits = 2) %>%
  group_rows("Count",1,4) %>%
  group_rows("Probability of zero",5,8) %>%
  group_rows("Estimated mean and variance",9,12) %>%
  group_rows("Model performance",13,13) %>%
  column_spec(2:6, width = "2cm") %>%
  kable_styling()
```
```{r comparemodels2, include=TRUE, message = FALSE, fig.cap = "Graphical comparison of the sample probabilities and the predicted probabilities."}
data %>% group_by(race, resp) %>%
  summarize(n = n()) %>% 
  ungroup() %>% 
  group_by(race) %>%
  ...
  pivot_longer(cols = 4:8, names_to = "model", values_to = "probability") %>%
  ggplot() + geom_line(aes(x=resp, y=probability, color = model)) +
  geom_point(aes(x=resp, y=probability, color = model), fill = NA, shape = 21) +
  facet_grid(cols = vars(race)) +
  theme_bw()
```
