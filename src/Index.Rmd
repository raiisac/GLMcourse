...
## Poisson model
Since the number of homicide victims a person knows is count data, a Poisson model is first applied to the data (Table \ref{tab:comparemodels}). 
...
Figure \@ref(fig:comparemodels3) compares the true data with the predicted probabilities.
...
```{r poissonpredict, include=FALSE, message = FALSE, fig.cap = "True and predicted probabilities of respondents knowing a certain number of homicide victims, for each race.", fig.width = 10, fig.height = 6}
...
comparison <- data.frame(data = unlist(c(NA, NA, NA, NA, NA, NA, 
                         ...
                         negbin = c(sprintf("%.2f (%.2f) %s",intercept.nb, 
                                    ...
                                    sprintf("%.2f (%.2f)",round(mod.nb$theta,2), round(sumnb$SE.theta,2)), 
                                    NA, NA, NA, 
...
rownames(comparison) <- c('Intercept', 'racewhite', 'theta','psi','Intercept ', 'racewhite ', 
...
A %>%
  ...
  theme_bw() + theme(legend.position = c(0.85, 0.7)) -> p1
```

```{r comparemodels3, include=TRUE, message = FALSE, warning = FALSE, fig.cap = "Graphical comparison of the difference between the sample probabilities and the predicted probabilities." , fig.height = 8, fig.width = 8}
...
A %>% filter(model != "data") %>%
  ...
  ylab("Predicted probability minus sample probability")-> p2
