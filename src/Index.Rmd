...
munbblack <- round(exp(unlist(mod.nb$coefficients[1])), digits = 2)
thetanb <- mod.nb$theta
munbwhite <- round(exp(unlist(mod.nb$coefficients[1]) + unlist(mod.nb$coefficients[2])), digits = 2)
aic.nb <- AIC(mod.nb)
...
The model shows similar estimated coefficients but much larger standard deviations than the Poisson model (table \@ref(tab:negbin)). The variance for each of the races can be obtained from the equation $\sigma^2 = \mu+\frac{1}{\theta}\mu^2$ where $\mu = e^{x' \hat{\beta} }$. For black people, $\mu =$ `r unname(munbblack)` and the variance is `r round(unname(munbblack + 1/thetanb*munbblack^2), digits = 2)`. For white people, $\mu =$ `r unname(munbwhite)` and the variance is `r round(unname(munbwhite + 1/thetanb*munbwhite^2), digits = 2)`. This shows that the variance for black people is overestimated (it was `r summ[1,5]` in reality) and the variance for white people is slightly underestimated (it was `r summ[2,5]` in reality).
...
The zero-inflated poisson model shows that white people are significantly more likely to know no homicide victims and the poisson regression coefficient for white people is still negative  and highly significant. The mean and variance are calculated as $\mu_i = (1-\pi_i)\lambda_i$ and $\sigma^2_i = (1-\pi_i)(\lambda^2_i + \lambda_i)$ with $i \in$ white,black where $\lambda$ is the average rate in the count process and $\pi$ the probability of zero. This yields $\mu_{black}=$ `r round((1-piblack.zip)*lambdablack.zip, digits=2)` and $\sigma^2_{black} =$ `r round((1-piblack.zip)*(lambdablack.zip^2 + lambdablack.zip), digits=2)` for black people and $\mu_{white}=$ `r round((1-piwhite.zip)*lambdawhite.zip, digits=2)` and $\sigma^2_{white} =$ `r round((1-piwhite.zip)*(lambdawhite.zip^2 + lambdawhite.zip), digits=2)` for white people.
...
# Conclusion {#CON}
Table \@ref(tab:comparemodels) shows the 
